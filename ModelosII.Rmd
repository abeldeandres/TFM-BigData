...
model.LR <- caret::train(outcome~.,data = train, method="glm", family="binomial",
...
modelGBM = caret::train(outcome~., data=train, method="gbm",distribution="bernoulli",metric=metric, trControl=fitControl, verbose=F )
...
ControlParamteres <- trainControl(method = "cv",
 ...
 modelxgboost <- caret::train(outcome~., 
                  data = train,
                  method = "xgbTree",
...
mod_fit_12<- caret::train(outcome ~ age,data = train, method="glm", family="binomial",
                 trControl = ctrl, tuneLength = 5,metric="ROC")
dfarray <- caret::list(mod_fit_Complete,mod_fit_1,mod_fit_2,mod_fit_3,mod_fit_4,mod_fit_5,mod_fit_6,mod_fit_7,mod_fit_8,mod_fit_9,mod_fit_10,mod_fit_11,mod_fit_12) 

dfPredictArray<-caret::list(pred.NN1,pred.NN2,pred.XGB,pred.GBM,pred.ada,pred.RF,pred.NB,pred.LR1)
...
algorithmList <- c("glm","nb","rf","ada","gbm","xgbTree", "blackboost","parRF","mlpWeightDecay","knn","earth","svmRadial","gam","glmnet")
set.seed(42)
models <- caretList(outcome~., data=train, trControl=trainControl, methodList=algorithmList)
