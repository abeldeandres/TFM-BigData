...
model.LR <- train(outcome~.,data = train, method="glm", family="binomial",
...
confusionMatrixLOG<-confusionMatrix(data = pred.LR, reference = test$outcome,positive="F")
confusionMatrixLOG
```
...
ctrl <- trainControl(method = "cv", number = 10, savePredictions = TRUE,classProbs=TRUE,
summaryFunction=twoClassSummary)
model.LR1 <- train(outcome ~ age+ec+eye+motor+verbal+pupils+phm+sah+oblt+mdls+hmt,data = train, method="glm", family="binomial",trControl = ctrl, tuneLength = 5,metric="ROC")
summary(model.LR1)
```
...
confusionMatrixLOG2<-confusionMatrix(data = pred.LR1, reference = test$outcome,positive="F")
confusionMatrixLOG2
...
axis(1,at=1:length(nameVector),labels=nameVector, las = 2)
axis(2,labels=FALSE)
aucArrStr<-strtrim(as.character(auc), 6) 
