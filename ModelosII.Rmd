...
time <- system.time(model.LR <- caret::train(outcome~.,data = train, method="glm", family="binomial",
                 trControl = ctrl, tuneLength = 5,metric="ROC"))
arrayTiempos<-c(arrayTiempos, as.numeric(time[3]))
summary(model.LR)
```
...
```{r , echo=FALSE}
pred.LR = predict(model.LR, newdata=test)
...
```{r , echo=FALSE}
set.seed(42)
time<-system.time(model.ada<-ada(outcome~.,data=train,iter=141,nu=0.3,control=rpart.control(maxdepth=1)))
arrayTiempos<-c(arrayTiempos, as.numeric(time[3]))
print(model.ada)
```
...
plot(arrayTiempos,xaxt="n",ann=FALSE,type="b", col="blue")
axis(1,at=1:length(nameVectorTime),labels=nameVectorTime, las = 2)
axis(2,labels=FALSE)
TimeArrStr<-strtrim(as.character(arrayTiempos), 6) 
text(arrayTiempos, TimeArrStr, labels=arrayTiempos, cex= 0.7, pos=1)
```
...
```{r warning=FALSE, , echo=FALSE}
set.seed(42)
stack.glm <- caretStack(models, method="gbm", metric="Accuracy", trControl=trainControl)
print(stack.glm)
```