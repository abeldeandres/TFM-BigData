...
* Los parametros con los que hemos hecho la fuerza bruta ha sido con los siguientes:
  + iter: Se ha iterado desde 1 hasta 200 de diez en diez.
  + maxdepth: Se ha iterado desde 1 hasta 10 de uno en uno.
  + nu: Se ha iterado desde 0.1 a 0.4 de 0.1 en 0.1.
...
confusionMatrixNET<-confusionMatrix(data=pred.NN1,reference=test$outcome,positive="F")
confusionMatrixNET
```
...
```
```{r , echo=FALSE}
pred.NN1<-predict(model.nn,test,type="prob")
plot.roc(test$outcome,pred.NN1$F,          # data
         percent = TRUE,                    # show all values in percent
         partial.auc=c(100, 90), 
         partial.auc.correct=TRUE,          # define a partial AUC (pAUC)
         print.auc=TRUE,                    
...
plot.roc(test$outcome, pred.NN1$F,
         percent = TRUE, 
         add = TRUE, 
         type = "n",                        # add to plot, but don't re-add the ROC itself (useless)
         partial.auc = c(100, 90), 
...
dfPredictArray<-list(pred.NN1,pred.NN2,pred.XGB,pred.GBM,pred.ada,pred.RF)
...
nameVector<-c("NN","NN2","XGBoost","GBM","Adaboost","R Forest","mod_fit_Complete","mod_fit_1","mod_fit_2","mod_fit_3","mod_fit_4","mod_fit_5","mod_fit_6","mod_fit_7","mod_fit_8","mod_fit_9","mod_fit_10","mod_fit_11","mod_fit_12")
...
nameVectorConf<-c("NN","NN2","XGBoost","GBM","Adaboost","R Forest","Logistic Regression Comp","Logistic Regression")
dfConfMatArray <- list(confusionMatrixNET,confusionMatrixNET2,confusionMatrixGBOOST,confusionMatrixGBM,confusionMatrixAdaBoost,confusionMatrixRF,confusionMatrixLOG,confusionMatrixLOG2) 
accuracyArr<-c()
kappa<-c()
...
plot(accuracyArr,xaxt="n",type="b", col="blue")
