...
###FASE I: Clasificaci贸n entre: ALIVE, DEATH, NO-DATA y MD/GR
* **En esta fase, se ha realizado una clasificacion de los datos dados seg煤n 4 resultados finales:**
...
```
###FASE II: Clasificaci贸n entre: ESCANEADOS y NO ESCANEADOS

```{r ,echo=FALSE}
datos.modelo$ESTADOESCANER<-NA

```
* En primer lugar, se han encontrado ciertos datos anomalos, en los que aparecen datos escaneados (1) y no tienen los datos del escaner, entonces deberiamos ponerlo como no escaneado (2).
```{r ,echo=FALSE}
...
* A continuaci贸n se van a clasificar los datos como:

  + Escaneados
```{r ,echo=FALSE}
head(datos.modelo[which(datos.modelo$ESTADOESCANER=="SCANEADO"  ),c(10,11,20,21,32)])

```
  + No escaneados
```{r ,echo=FALSE}
head(datos.modelo[which(datos.modelo$ESTADOESCANER=="NOSCANEADO"  ),c(10,11,20,21,32)])

```
  + En analisis
```{r ,echo=FALSE}
head(datos.modelo[which(datos.modelo$ESTADOESCANER=="ENANALISIS"  ),c(10,11,20,21,32)])

```

* Si el Outcome es 2 (el paciente se ha transferido a otro hospital), se ha escaneado en dicho hospital (TH_SCAN) y no se tiene ninguna informaci贸n en los esc谩neres, se clasificaran como "En an谩lisis".
...
```
* Sobre el dataset NOSCANEADO: Si el Outcome es 2 (el paciente se ha transferido a otro hospital) y no se ha realizado ningun escaner pero si contiene datos en el escaner, entonces se clasificara como "Escaneado".
```{r ,echo=FALSE}
...
```
* Sobre el dataset NOSCANEADO: Nos hemos dado cuenta que existen datos anomalos, que contienen varios escaneres, pero sin embargo, no se indica como escaneado, son los registros: 2628,3276,3279,8469,8655, etc. (En total son 12)
```{r ,echo=FALSE}
head(datos.modelo[which(datos.modelo$ESTADOESCANER == "NOSCANEADO"  
                           & !is.na(datos.modelo$EO_1.or.more.PH)
                           & !is.na(datos.modelo$EO_Subarachnoid.bleed)
...
* Sobre el dataset ENANALISIS: Nos hemos dado cuenta de que existen datos anomalos. Para las variables de los pacientes que se han transferido a otro hospital (TH), existen variables de escaner (TH_Head.CT.scan) que se encuentran vacias, junto con el resto de variables del escaner en particular. Por lo tanto, se ha asignado el valor de 2 a la variable de escaner (TH_Head.CT.scan) y se han incluido en los escaneados, puesto que en todos ellos, en la variable EO_Head.CT.scan si que existe un valor de 1 (escaneados) y no se han encontrado mas anomalias en dichos datos.
```{r ,echo=FALSE}
...
```
* Sobre el dataset SCANEADO: Se van a eliminar todas las filas que no tengan informacion en el TH_Major.EC.injury y en el EO_Major.EC.injury
...
```
* Sobre el dataset SCANEADO: Comprobamos que las variables: EO_Cause y EO_Symptoms, no contengan valores nulos.
```{r ,echo=FALSE}
...
```
* Sobre el dataset SCANEADO: Comprobamos que las variable: EO_Outcome no se encuentre nula.(En total son 2 registros)
```{r ,echo=FALSE}
...
* Sobre el dataset SCANEADO: Comprobamos que existe un valor an髆alo (que se sale del rango) en un registro en la columna de EO_Major.EC.Injury. Este valor lo cambiaremos a positivo -> 1
```{r ,echo=FALSE}
...
### FASE III: Eliminacion y centralizacion de variables
...
```
* Como se puede observar, podriamos prescindir de la variable TH_Cause, puesto que recoge la misma informaci髇 que EO_Cause.

* A continuaci髇 vamos a aunar todas las variables del escaner. Si un paciente ha sido transferido a otro hospital y se han realizado los escaneres en dicho hospital, entonces, se mantendran los ultimos valores del escaner. En caso contrario, se usaran los primeros resultados de escaner obtenidos en el primer. Adem醩, eliminaremos todas las variables que no se utilicen.
